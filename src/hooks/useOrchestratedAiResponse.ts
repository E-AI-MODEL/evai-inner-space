...
export function useOrchestratedAiResponse(apiKey: string, apiKey2?: string) {
  ...
  const analysisPhase = (messages: Message[]): ChatHistoryItem[] => {
    return messages.slice(-6).map(m => ({
      ...
      content: m.content,
      from: m.from // Add from property for compatibility
    }));
  };
      ...
      if (analysis) {
        ...
        if (analysis.seedSuggestion && Math.random() > 0.3) {
          ...
          if (generatedSeedResponse) {
            ...
            if (injected) {
              console.log('✅ Auto-generated seed injected:', analysis.seedSuggestion);
            }
          }
        }

        // Enhance the response with secondary insights
        if (analysis.insights.length > 0 && analysis.confidence > 0.7) {
          const enhancement = analysis.insights[0];
          enhancedResponse = `${primaryResponse.response}\n\n*${enhancement}*`;
        }
      }

      return { enhancedResponse, gapAnalysisFeedback, autoGeneratedSeed };
      
    } catch (error) {
      console.error('🔴 Enhancement phase error:', error);
      return { 
        enhancedResponse: primaryResponse.response,
        gapAnalysisFeedback: ''
      };
  ...
  const packagingPhase = (
    ...
    from: 'ai',
    label: ai.label,
    content: enhancement.enhancedResponse + enhancement.gapAnalysisFeedback,
    emotionSeed: ai.emotion,
    animate: true,
    timestamp: new Date(),
    feedback: null,
    symbolicInferences: enhancement.autoGeneratedSeed ? 
      [`Auto-seed: ${enhancement.autoGeneratedSeed.emotion}`, ...(ai.symbolicInferences || [])] : 
      ai.symbolicInferences,
  });
